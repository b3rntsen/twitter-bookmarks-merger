"""
Management command to test list processing through the queue.
"""
...
class Command(BaseCommand):
    help = 'Test list processing by creating a job and queuing it through Django-Q'

    def add_arguments(self, parser):
        parser.add_argument(
            '--username',
            type=str,
            help='Username to process lists for (defaults to first user with Twitter profile)',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Testing list processing for user: {user.username}"))
        ...
        if existing_job:
            self.stdout.write(self.style.WARNING(f"Found existing list job {existing_job.id}, resetting it..."))
            ...
            self.stdout.write(f"Creating and queuing list job for {today}...")
            ...
            # Filter for list job
            list_jobs = [j for j in jobs if j.content_type == 'lists']
            if not list_jobs:
                self.stdout.write(self.style.ERROR("Failed to create list job"))
                return
            job = list_jobs[0]
        ...
        if job.status == 'pending':
            ...
            self.stdout.write(self.style.SUCCESS(f"✓ List job {job.id} queued"))
        else:
            self.stdout.write(self.style.SUCCESS(f"✓ List job {job.id} is {job.status}"))
        
        self.stdout.write(f"  Status: {job.status}")
        self.stdout.write(f"  Processing date: {job.processing_date}")
        self.stdout.write("")
        self.stdout.write("Monitor progress with:")
        self.stdout.write(f"  tail -f qcluster.log | grep -E '\\[LISTS\\]|\\[TASK\\]'")
        self.stdout.write("")
