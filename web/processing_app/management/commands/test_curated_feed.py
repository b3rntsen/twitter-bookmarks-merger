"""
Management command to test curated feed processing through the queue.
"""
...
class Command(BaseCommand):
    help = 'Test curated feed processing by creating a job and queuing it through Django-Q'

    def add_arguments(self, parser):
        parser.add_argument(
            '--username',
            type=str,
            help='Username to process curated feed for (defaults to first user with Twitter profile)',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Testing curated feed processing for user: {user.username}"))
        ...
        if existing_job:
            self.stdout.write(self.style.WARNING(f"Found existing curated feed job {existing_job.id}, resetting it..."))
            ...
            self.stdout.write(f"Creating and queuing curated feed job for {today}...")
            ...
            # Filter for curated feed job
            curated_feed_jobs = [j for j in jobs if j.content_type == 'curated_feed']
            if not curated_feed_jobs:
                self.stdout.write(self.style.ERROR("Failed to create curated feed job"))
                return
            job = curated_feed_jobs[0]
        ...
        if job.status == 'pending':
            ...
            self.stdout.write(self.style.SUCCESS(f"✓ Curated feed job {job.id} queued"))
        else:
            self.stdout.write(self.style.SUCCESS(f"✓ Curated feed job {job.id} is {job.status}"))
        
        self.stdout.write(f"  Status: {job.status}")
        self.stdout.write(f"  Processing date: {job.processing_date}")
        self.stdout.write("")
        self.stdout.write("Monitor progress with:")
        self.stdout.write(f"  tail -f qcluster.log | grep -E '\\[CURATED_FEED\\]|\\[TASK\\]'")
        self.stdout.write("")
